...
int main(int argc, char **argv) {
  ...
  if (std::filesystem::is_directory(p)) {
    for (const auto &entry : std::filesystem::directory_iterator(p)) {
      ...
      if (filename.find("kernelslist") != std::string::npos &&
          entry.path().extension() != ".g") {
        kernelslist_list.push_back(entry.path().string());
      }
    }
  } else if (std::filesystem::is_regular_file(p)) {
    if (p.extension() == ".g") {
      cerr << "Input kernelslist already post-processed (.g). Skipping: " << p
           << "\n";
      return 0;
    }
  ...
  for (auto kernellist_filepath : kernelslist_list) {
    ...
    if (!ofs.is_open()) {
      cerr << "Unable to open output file in directory: " << directory << endl;
      return 1;
    }
...
void group_per_block(const char *filepath) {
  ...
  bool passthrough_mode = false; // some traces are already grouped, just copy
  ...
  while (!cin.eof()) {
    ...
    if (line.length() == 0 || line[0] == '#') {
      ...
      if (!std::isdigit(line[0])) {
        cerr << "Warning: unexpected trace format detected in " << filepath_str
      ...
      if (!(ss >> tb_id_x >> tb_id_y >> tb_id_z >> warpid_tb)) {
        cerr << "Warning: failed to parse thread/warp info in trace "
             << filepath_str << ", skipping line: " << line << "\n";
        continue;
      }

      if (!found_grid_dim || !found_block_dim || insts.empty()) {
        cerr << "Warning: missing grid/block dimensions for trace "
             << filepath_str << ", skipping line: " << line << "\n";
        continue;
      }
      ...
      if (tb_id >= insts.size()) {
        cerr << "Warning: thread block id " << tb_id
             << " exceeds allocated blocks for trace " << filepath_str
             << ", skipping line: " << line << "\n";
        continue;
      }
      ...
      if (opcode.find("LDGSTS") != string::npos) {
        if (warpid_tb >= ldgsts_flags[tb_id].size()) {
          cerr << "Warning: warp id " << warpid_tb
               << " exceeds allocated warps for trace " << filepath_str
               << ", skipping line: " << line << "\n";
          continue;
        }
