...
static void enter_kernel_launch(CUcontext ctx, CUfunction func,
  ...
  // TODO lyq
  char sass_buffer[1024];
  sprintf(sass_buffer, std::string(traces_location + "/kernel-%d.sass").c_str(), ctx_kernelid[ctx]);
  char mem_buffer[1024];
  sprintf(mem_buffer, std::string(traces_location + "/kernel-%d.mem").c_str(), ctx_kernelid[ctx]);
  ...
  if (!stop_report) {
    ...
    // TODO lyq
    sassFile = fopen(sass_buffer, "w");
    memFile = fopen(mem_buffer, "w");
    // TODO
    printf("Writing results to %s\n", sass_buffer);
    fprintf(sassFile, "SwiftSim sass trace\n");
    fprintf(sassFile, "kernel_id = %d\n", ctx_kernelid[ctx]);
    grid_dimx = gridDimX;
    grid_dimy = gridDimY;
    grid_dimz = gridDimZ;
    fprintf(sassFile, "grid_size = %d\n", gridDimX * gridDimY * gridDimZ);
    fprintf(sassFile, "block_size = %d\n", blockDimX * blockDimY * blockDimZ);
    fprintf(sassFile, "shared_mem_bytes = %d\n",
            shmem_static_nbytes + sharedMemBytes);
    fprintf(sassFile, "num_registers = %d\n", nregs);
    fprintf(sassFile, "SwiftSim kernel info end\n");
  }
  ...
  while (recv_thread_started) {
    ...
    if (recv_thread_receiving &&
      ...
      while (num_processed_bytes < num_recv_bytes) {
        ...
        // TODO lyq
        int block_id = ma->cta_id_z * grid_dimx * grid_dimy + ma->cta_id_y * grid_dimx + ma->cta_id_x;
        if (ma->is_mem || id_to_opcode_map[ma->opcode_id].c_str() == "ATOM" || id_to_opcode_map[ma->opcode_id].c_str() == "ATOMS" || id_to_opcode_map[ma->opcode_id].c_str() == "ATOMG") {
          fprintf(memFile, "%d ", block_id);
          fprintf(memFile, "%d ", ma->warpid_tb);
          fprintf(memFile, "%04x ", ma->vpc);
          fprintf(memFile, "%s ", id_to_opcode_map[ma->opcode_id].c_str());
        }
        //  if(block_id == 0){
        fprintf(sassFile, "%d ", block_id);
        fprintf(sassFile, "%d ", ma->warpid_tb);
        fprintf(sassFile, "%04x ", ma->vpc);
        fprintf(sassFile, "%08x ", ma->active_mask & ma->predicate_mask);
        if (ma->GPRDst >= 0) {
          fprintf(sassFile, "1 ");
          fprintf(sassFile, "R%d ", ma->GPRDst);
        }
        else
          fprintf(sassFile, "0 ");
        fprintf(sassFile, "%s ", id_to_opcode_map[ma->opcode_id].c_str());
        unsigned src_counts = 0;
        for (int s = 0; s < MAX_SRC; s++) // GPR srcs count.
          if (ma->GPRSrcs[s] >= 0)
            src_counts++;
        fprintf(sassFile, "%d ", src_counts);
        for (int s = 0; s < MAX_SRC; s++) // GPR srcs.
          if (ma->GPRSrcs[s] >= 0)
            fprintf(sassFile, "R%d ", ma->GPRSrcs[s]);
        fprintf(sassFile, "\n");
        ...
        if (ma->is_mem) {
          for (int s = 0; s < 32; s++) {
            if (mask.test(s))
              fprintf(memFile, "0x%016lx ", ma->addrs[s]);
          }
          fprintf(memFile, "\n");
        }
        num_processed_bytes += sizeof(inst_trace_t);
