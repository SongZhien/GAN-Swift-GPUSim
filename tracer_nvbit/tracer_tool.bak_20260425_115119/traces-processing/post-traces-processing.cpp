...
void group_per_block(const char *filepath) {
  ...
  bool input_grouped_format = false;
  unsigned current_tb_id_x = 0, current_tb_id_y = 0, current_tb_id_z = 0;
  unsigned current_tb_linear = 0;
  unsigned current_warpid = 0;
  bool has_current_tb = false;
  bool has_current_warp = false;
  ...
  while (!cin.eof()) {
    ...
    if (line.length() == 0 || line[0] == '#') {
      ...
      cout << line << endl;
      continue;
    } else if (line.rfind("thread block", 0) == 0) {
      input_grouped_format = true;
      if (sscanf(line.c_str(), "thread block = %u,%u,%u", &current_tb_id_x,
                 &current_tb_id_y, &current_tb_id_z) == 3) {
        current_tb_linear = current_tb_id_z * grid_dim_y * grid_dim_x +
                            current_tb_id_y * grid_dim_x + current_tb_id_x;
        has_current_tb = true;
        has_current_warp = false;
      } else {
        cerr << "Warning: Unable to parse thread block line: " << line << "\n";
        has_current_tb = false;
      }
      continue;
    } else if (line.rfind("warp", 0) == 0) {
      input_grouped_format = true;
      if (sscanf(line.c_str(), "warp = %u", &current_warpid) == 1) {
        has_current_warp = true;
      } else {
        cerr << "Warning: Unable to parse warp line: " << line << "\n";
        has_current_warp = false;
      }
      continue;
    } else if (line.rfind("insts", 0) == 0) {
      input_grouped_format = true;
      continue;
    } else {
      ...
      string rest_of_line;
      if (!found_grid_dim || !found_block_dim) {
        cerr << "Warning: grid/block dim missing before instruction line: "
             << line << "\n";
        continue;
      }

      if (input_grouped_format) {
        if (!has_current_tb || !has_current_warp) {
          cerr << "Warning: Missing thread block/warp context for line: "
               << line << "\n";
          continue;
        }
        tb_id = current_tb_linear;
        warpid_tb = current_warpid;
        rest_of_line = line;
      } else {
        ss.str(line);
        ss >> tb_id_x >> tb_id_y >> tb_id_z >> warpid_tb;
        tb_id = tb_id_z * grid_dim_y * grid_dim_x + tb_id_y * grid_dim_x +
                tb_id_x;
      ...
      if (tb_id >= insts.size()) {
        cerr << "Warning: Thread block id out of range (" << tb_id
             << " >= " << insts.size() << ") for line: " << line << "\n";
        continue;
      }
      if (warpid_tb >= insts[tb_id].warp_insts_array.size()) {
        cerr << "Warning: Warp id out of range (" << warpid_tb
             << " >= " << insts[tb_id].warp_insts_array.size() << ") for thread block " << tb_id << "\n";
        continue;
      }

      unsigned block_x = input_grouped_format ? current_tb_id_x : tb_id_x;
      unsigned block_y = input_grouped_format ? current_tb_id_y : tb_id_y;
      unsigned block_z = input_grouped_format ? current_tb_id_z : tb_id_z;

      if (!insts[tb_id].initialized) {
        insts[tb_id].tb_id_x = block_x;
        insts[tb_id].tb_id_y = block_y;
        insts[tb_id].tb_id_z = block_z;
